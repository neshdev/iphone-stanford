                ...
                if let endIndex = (indices?.lastObject as? NSNumber)?.integerValue {
                    ...
                    if length > 0 {
                        ...
                        if end > start {
                            ...
                            var cond = keyword.hasPrefix(prefix!)
                            
                            if (prefix != nil) && (start > 0) && !cond {
                                keyword = inText.substringWithRange(inText.startIndex.advancedBy(start-1)...inText.startIndex.advancedBy(end-2))
                            }
